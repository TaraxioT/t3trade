...
Treat a base directory as disposable only when it was created or deliberately selected for the current test. Never delete or directly seed the shared `~/.t3trade` directory. Prefer starting with a new temporary base directory over clearing state of uncertain ownership.
...
The helper refuses to write to the shared `~/.t3trade` directory by default and creates a database backup before each mutation.
