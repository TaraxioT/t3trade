...
Verify Docker is usable on the actual demo machine; build/pin a trusted runner image and execute a tiny contained typed function with a piped JSON stdin/output round trip and network/filesystem probes. Select one supported already-configured in-app provider and measure a small artifact-producing turn in a disposable fixture, not this checkout. Agree the proposed UI direction; U0 backend-handoff verification runs after F3. Record actual endpoint identifiers, schema differences, three pool IDs, thresholds/window/freshness settings, testnet config and remaining blockers. Freeze defaults rather than proliferating options. Do not deploy a pool yet.
...
George opens one captured Graph swap in an explorer and compares pool/token identities; checks block timestamp/lag; sees a sandbox isolation result; confirms the detector is absent. Review actual testnet chain/token/owner/operator/spend limits and explicitly accept or amend them. Inspect the proposed focused layout; this does not complete later U0. Record F0 PASS with the concrete parameters, or list blockers. Later signing remains disabled without this grant.
...
Owned files/scope: packages/trading-contracts/src/forge.ts + exports/tests; new apps/server/src/trading/forge/GraphSource.ts and tests; application owner alone handles AGENTS.md narrow target exception, package manifests/migrations and Forge transport registrations.

Implement the pinned-block adapter and exact token normalization from the plan. Host selects endpoint/credentials/three approved pools; validated query artifact defines allowed source operations. Do not use Hyperliquid data as Graph input. Implement bounded cursor pagination, deduplication, complete pre-window anchor-candidate coverage (5-minute age cap, not just the nearest trade), exact quote micros, source block/hash/digest, and explicit complete/empty/stale/unavailable states. Keep credentials out of URLs and tool output.
Update the AGENTS.md execution-target sentence with the narrow Forge Sepolia exception authorized by this user request; retain all existing Hyperliquid safety constraints. Introduce runtime-validated Forge source/evaluation/job/policy schemas, with environment/thread identity and separate observation/execution chain labels. Keep TradingVenue hyperliquid-only. Add the small persistent evidence/observation boundary using existing migration conventions; raw evidence is accessed by ID through authenticated transport, not stuffed into every event. Ensure source-only reads do not call Hyperliquid or require a signer.
...
UI HANDOFF (required, see revised unified-graph-ui-plan.html): provide source/thread discovery before installation and without an HL market row; authenticated bounded real per-pool chart series with explicit UTC domain, coverage, quote units, normalization anchors, gaps, provenance IDs and source-health metadata. Do not provide only latest categorical readings or coerce pool activity to Hyperliquid candles. Register schemas/server/client-runtime read paths. Exact equivalent API names are acceptable; F4/U0 checks the actual handoff.
...
UI HANDOFF: expose persisted source-inspection/build/test/install receipts and immutable versions via authenticated bounded reads. Extend generated SDK/evaluation evidence to report actual per-pool qualifying/excluded counts, quote volumes, anchors and source trade IDs so F5/U2 can explain revisions. Validate references against input. Keep a Forge source/build context discoverable when HL threadMarket is null. No hardcoded detector diagnostics or client reconstruction.
...
UI HANDOFF: expose exact pool proposal/grant, position/liquidity, direct controls, local publication state, chain policy snapshot with as-of block/time and stale status, installed-vs-confirmed module hashes, submitted/confirmed/reverted/unknown tx and actual swap LP/total fee evidence. These must be transport data, not only CLI output. Do not infer freshly confirmed expiry from a client clock.
...
Owned files/scope: MarketChartPanel/MissionPriceChart/ThreadMarketCard/ThreadMarketPanel and bounded ChatView integration; new ForgeSignalLane/ForgeBuildReceipt/ForgePoolInspector/ForgeVersionDiff; web state + trading.css. Execute revised UI U0 then U1 here; they are sequential slices with human gates.

Use the revised unified-graph-ui-plan.html as the executable contract. Run U0 first to verify/add missing backend read models from F1–F3, then U1 to implement the focused layout. F0 visual approval is not U0 completion. Do not duplicate work if main-pack implementation already provides the required contracts.
Preserve the actual existing real-data graph: HL weekly candles, volume, EMA overlays, event markers, published scene count/selection/Open, Live/Calendar/Event aligned and source provenance. New pool activity has its own bounded real time-series renderer and signal lane in the same thread workspace. It does not replace HL data or inherit All/weekly history promises. Existing MissionPriceChart has no multi-series or general zoom API.
Patch ChatView at its existing main-content/instrument slot and composer drawer/companion mount points. Keep the current MessagesTimeline/ChatComposer instance and measured overlay logic, avoid duplicate charts/polls, preserve draft hero, right panel, terminal, pending questions and provider controls. Source/build discovery must work without HL threadMarket or account response. Focus-state selection and per-source range/scene state are scoped and restorable. Keep all existing exposure/working-order controls independently accessible.
Include the scoped maxBars refresh-identity repair and stale display in tradingMarketChartState/MarketChartPanel with targeted tests. Use measured available content width, focus-specific chart sizing and accessible inspector disclosure. Comparison remains unavailable until F5 supplies actual v2 diagnostics; do not invent a preview module or policy success.
Human U0 and U1 gates are the required sub-gates of this F4 phase. Later U2 belongs to F6, after F5. If the user prohibits runtime launch during execution, finish independent implementation/checks and explicitly leave integrated browser acceptance pending.
...
Run generated + independent acceptance tests and compare v1/v2 on the same authentic pinned historical window. Ensure v2 can select an earlier qualifying pre-window anchor when the nearest one is tiny; if new data fields or coverage are needed, revise and validate query.graphql too. Show exactly which trades/pools are excluded and whether classification changes; display historical provenance. Publish/install v2 via expected-v1 CAS; confirm source/module hash difference. A failed revision leaves v1 live.
...
UI HANDOFF: persist both sealed evaluation IDs referencing the identical authentic source window, and per-pool eligibility/volume/anchor/excluded-trade diagnostics. U1 can honestly show comparison unavailable before this phase; U2 requires the actual output. Historical comparison must never schedule a fee intent.

Required automated/live evidence:
Revision immutability/hash/CAS/race tests; tiny-trade and cumulative-floor boundary cases, including tiny nearest anchor with a valid earlier qualifying anchor; live-vs-history policy guard; real provider revision; pinned real comparison; confirmed v2 policy + swap receipts and two-fee evidence or precise blocker.
...
UNIFICATION GATE: complete revised U2 after actual U0/U1 and F5. Recheck existing weekly candles, EMA, event scenes, source switching, no-HL-focus entry, composer/terminal/right-panel layout and direct risk controls. If the UI pack is executed after F7, reverify affected final demo evidence once; do not repeat pool deployment by rote.
