...
- Dev commands run from a linked **git worktree** default to that worktree's gitignored `.t3`, even
  ...
  precedence.
- From the **main checkout**, dev commands implicitly use `~/.t3trade/dev`, keeping development state
  separate from `~/.t3trade/userdata`. An explicit `--home-dir <path>` stores state under
