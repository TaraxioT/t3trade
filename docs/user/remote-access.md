...
`t3 pair` finds the running server (the shared `~/.t3trade` install, or the current worktree's dev server when run inside one), issues a one-time pairing token, and prints the pairing URL as a QR code you can scan from your phone.
...
- HTTPS/WSS-compatible endpoints work from `https://app.athelstan.xyz`, but are not made the default
  automatically.
...
If the copied link points directly at `http://192.168.x.y:3773`, open it from a client that can reach that LAN address. If it points at `https://app.athelstan.xyz/pair?...`, the hosted web app will save the environment and connect directly to the backend URL in the link.
...
For `https://app.athelstan.xyz`, prefer an HTTPS Tailnet or other HTTPS endpoint. A plain `http://100.x.y.z:3773` endpoint can still work from a desktop client or another browser page served over HTTP, but it will not work from the hosted HTTPS app because of browser mixed-content rules.
...
The desktop SSH launcher connects with a non-interactive `sh` session, writes a small launcher script under `~/.t3trade/ssh-launch/<host-key>/`, starts or reuses a remote T3 server, and forwards the remote loopback port back to your desktop.
...
The hosted web app at `https://app.athelstan.xyz` can save a remote backend in browser local storage from a URL like:

```text
https://app.athelstan.xyz/pair?host=https://backend.example.com:3773#token=PAIRCODE
```
