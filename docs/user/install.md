...
Choose a WSL distro in **Settings → Connections** to run agents and projects there.

When the desktop app runs a WSL backend, it installs the matching server runtime into
`~/.t3trade/wsl-runtime` inside the selected distro. The first launch after installing or updating T3
Trade may take a little longer while that release's runtime is extracted. Later launches reuse the
