...
const ALL_PLAYBOOKS: ReadonlyArray<Playbook> = [
  ...
  },
  {
    ...
    procedure: [
      ...
      "SIZE THE NOTIONAL TO THE TARGET, NOT THE TARGET TO THE NOTIONAL. The target is a USD figure, the move is a percentage, and the round trip is also a percentage of notional — so the notional is what converts one into the other: notional x (expected move % - round trip %) = the target. Sizing DOWN does not make a target cheaper to reach, it makes it arithmetically unreachable, because the costs shrink with the position and the target does not. trading_enter does NOT size up for you: a `sizeEth`/`notionalUsd` you name is a ceiling and it is honoured. It checks instead, and `notes` tells you when the size on offer is short of the published target. That is a target to re-cut off the move the market is actually producing, or a trade to take for a nearer rung — it is NOT a reason to decline a setup whose move pays the round trip. If you want the target funded, omit the size and take the ceiling.",
