...
describe("WSL runtime cache", () => {
  ...
  it("installs through a temporary directory and only reuses valid completed caches", () => {
    ...
    expect(script).toContain('runtime_parent="$HOME/.t3trade/wsl-runtime"');
  ...
  it("re-hashes the server entry against the digest the install recorded", () => {
    ...
    expect(script).toContain(
      `printf '%s\\n' "$installed_entry_digest" > "$runtime_tmp/.t3trade-wsl-runtime-ready"`,
    );
    ...
    const markerWritten = script.indexOf('> "$runtime_tmp/.t3trade-wsl-runtime-ready"');
  ...
  it("parses only absolute Linux runtime paths", () => {
    expect(parseWslRuntimeRoot("runtimeRoot:/home/josh/.t3trade/wsl-runtime/1.2.3-x64\n")).toBe(
      "/home/josh/.t3trade/wsl-runtime/1.2.3-x64",
    );
  ...
  it("prunes completed runtimes except the current and newest previous cache", () => {
    ...
    expect(script).toContain('[ -f "$candidate/.t3trade-wsl-runtime-ready" ] || continue');
  ...
  it("invalidates a cache by dropping its ready marker, not the tree", () => {
    ...
    expect(script).toContain(
      'rm -f "$HOME/.t3trade/wsl-runtime/1.2.3_x64/.t3trade-wsl-runtime-ready"',
    );
...
describe.skipIf(posixShellRunner === null)("WSL runtime install script (executed)", () => {
  ...
  const createFixture = () => {
    ...
    return {
      ...
      archiveSha,
      runtimeId,
      runtimeParent: `${work}/home/.t3trade/wsl-runtime`,
      runtimeRoot: `${work}/home/.t3trade/wsl-runtime/${runtimeId}`,
      serverEntry: `${work}/home/.t3trade/wsl-runtime/${runtimeId}/apps/server/dist/bin.mjs`,
      installScript,
  ...
  it("leases a warm cache across the prepare-to-spawn handoff", () => {
    ...
    const selected = runShell(
      [
        ...
        'printf ready > "$runtime_parent/sha256-current/.t3trade-wsl-runtime-ready"',
        'printf ready > "$runtime_parent/sha256-previous/.t3trade-wsl-runtime-ready"',
  ...
  it("prunes a selected cache after its prepare-to-spawn grace period expires", () => {
    ...
    const result = runShell(
      [
        ...
        'printf ready > "$runtime_parent/sha256-previous/.t3trade-wsl-runtime-ready"',
        `touch -d "10 minutes ago" ${sh(fixture.runtimeRoot)}`,
        `touch -d "10 minutes ago" ${sh(`${fixture.runtimeRoot}/.t3trade-wsl-runtime-selected`)}`,
        'touch -d "1 minute ago" "$runtime_parent/sha256-previous"',
        `HOME=${sh(`${fixture.work}/home`)}`,
        "export HOME",
        buildWslRuntimePruneScript("sha256-current"),
  ...
  it("removes an aged stale tree after replacing an active unready cache", () => {
    ...
    const result = runShell(
      [
        ...
        `runtime_parent=${sh(fixture.runtimeParent)}`,
        'rm "$runtime_root/.t3trade-wsl-runtime-ready"',
  ...
  it("prunes old and markerless caches without touching retained, active, locked, or unrelated roots", () => {
    const result = runShell(
      [
        ...
        "work=$(mktemp -d)",
        'home="$work/home"',
        'runtime_parent="$home/.t3trade/wsl-runtime"',
        'mkdir -p "$runtime_parent"',
        'make_ready() { mkdir -p "$runtime_parent/$1/apps/server/dist"; printf ready > "$runtime_parent/$1/.t3trade-wsl-runtime-ready"; }',
