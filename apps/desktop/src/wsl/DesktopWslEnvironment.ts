...
// that install wrote.
const WSL_RUNTIME_READY_MARKER = ".t3trade-wsl-runtime-ready";
const WSL_RUNTIME_SELECTED_MARKER = ".t3trade-wsl-runtime-selected";
...
export const buildWslRuntimeInvalidateScript = (runtimeId: string): string => {
  ...
  return [
    "set -eu",
    `rm -f "$HOME/.t3trade/wsl-runtime/${safeRuntimeId}/${WSL_RUNTIME_READY_MARKER}"`,
  ].join("\n");
};
...
const formatNodePtyProbeFailureReason = (exitCode: number): string | null =>
  exitCode === NODE_PTY_PREBUILD_MISSING_EXIT_CODE
    ? "WSL support is missing from this T3 Trade build: the packaged Linux node-pty binary was not included. Rebuild the Windows artifact with `--wsl-prebuild <path-to-linux-pty.node>` or install a build that includes WSL support."
    : null;
