...
const defaultInput = {
  ...
  processArch: "arm64",
  appVersion: "0.0.22",
  appPath: "/Applications/T3 Trade.app/Contents/Resources/app.asar",
  isPackaged: false,
  resourcesPath: "/Applications/T3 Trade.app/Contents/Resources",
...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.appUserModelId, "com.t3trades.app.dev");
      assert.equal(environment.linuxWmClass, "t3trade-dev");
      assert.deepEqual(
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(development.stateDir, "/Users/alice/.t3trade/dev");
      assert.equal(production.stateDir, "/Users/alice/.t3trade/userdata");
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          T3CODE_DESKTOP_APP_USER_MODEL_ID: " com.t3trades.app.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.t3trades.app.dev.local");
    }),
  );
