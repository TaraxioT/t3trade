...
import type { OrchestrationTradingMission, ScopedThreadRef, ThreadId } from "@t3tools/contracts";
...
/**
 * The status hue of each mission state, following the reference design: orange
 * while the mission waits on the market, emerald while it holds exposure, red
 * when something stopped it, and muted once nothing is live any more. The
 * SidebarV2 version of this map coloured a dot; upstream's row colours the
 * label instead, so these are the text equivalents of the same semantics.
 */
const MISSION_STATUS_CLASS: Record<TradingMissionStatus, string> = {
  initializing: "text-sky-600 dark:text-sky-400",
  analysing: "text-sky-600 dark:text-sky-400",
  waiting: "text-orange-600 dark:text-orange-400",
  executing: "text-sky-600 dark:text-sky-400",
  position_open: "text-emerald-700 dark:text-emerald-300",
  paused: "text-muted-foreground",
  agent_unavailable: "text-red-700 dark:text-red-300",
  blocked: "text-red-700 dark:text-red-300",
  revoked: "text-red-700 dark:text-red-300",
...
const SidebarThreadRow = memo(function SidebarThreadRow(props: {
  ...
  // A mission's P&L is unrealised while it still holds exposure and realised
  // once it is flat with at least one fill behind it. No fills means there is
  // no number worth showing yet.
        ...
        : status === "approval"
          ...
          : status === "input"
            ...
            : status === "failed"
              ...
              : // A mission-bound thread reads its state from the mission
                // rather than from the session: "Holding · +$12.40" says more
                // than "Working", and it stays true while the agent is idle
                // between wakes. Approval/input/failed still win — those need
                // a human now. Unrealised P&L while exposed, realised once
                // the position is closed and something actually filled.
                mission !== null
                ? {
                    label: MISSION_STATUS_LABELS[mission.status],
                    icon: null,
                    className: MISSION_STATUS_CLASS[mission.status],
                  ...
                  {topStatus ? (
                    isWokeStatus ? (
                      ...
                      >
                        ...
                        <span role="status">{topStatus.label}</span>
                        {/* A mission's money rides along with its state, so
                            "Holding · +$12.40" reads as one status. */}
                        {"pnlLabel" in topStatus && topStatus.pnlLabel !== null ? (
                          <span aria-hidden>· {topStatus.pnlLabel}</span>
                        ) : null}
                        {status === "working" ? (
                          <span aria-hidden>
        ...
        fixedHeader={
          ...
          <SidebarGroup className="relative z-[1] gap-1 p-[var(--sidebar-content-inset)]">
            <div className="flex items-center gap-1">
              {/* focus-within carries the ring on the row, not the unstyled
                  input, so the visible focus outline survives the header's
                  background and cannot be clipped by it. */}
              <div className="flex h-8 min-w-0 flex-1 items-center gap-2 rounded-md px-2 py-1.5 text-sm font-medium text-sidebar-muted-foreground hover:bg-sidebar-row-hover hover:text-sidebar-foreground focus-within:outline-2 focus-within:-outline-offset-2 focus-within:outline-ring">
