...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  const timelineListFooter = useMemo(
    () => (
      <>
        {/* Mission execution cards pin to the end of the scroll, above the
            composer inset, so they scroll away with the conversation. */}
        {footerContent === undefined ? null : (
    ...
    [anchoredEndSpace, contentInsetEndAdjustment, footerContent],
  );
...
function UserTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "message" }> }) {
  ...
  // A wakeup snapshot arrives as a user message (§12.4) and renders as the
  // spine card instead of a bubble. The check sits below the hooks so none of
  // them is called conditionally.
...
/**
 * Copy control for an expanded tool body, without a Tooltip: base-ui's
 * floating-ui setup needs a DOM (`Element`), and this row renders under
 * react-test-renderer in node. The anchored toast already confirms the copy;
 * the button's aria-label covers the rest.
 */
const WorkEntryCopyButton = memo(function WorkEntryCopyButton({ text }: { text: string }) {
      ...
      {expanded && canExpand && expandedBody ? (
        ...
        >
          <div className="flex items-start gap-2">
            ...
            <pre className={cn(toolCallExpandedBodyClassName, "max-h-[32rem] flex-1")}>
              {expandedBody}
            </pre>
            <WorkEntryCopyButton text={expandedBody} />
          </div>
        </div>
      ) : null}
    </div>
