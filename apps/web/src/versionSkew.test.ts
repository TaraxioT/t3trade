...
const MISMATCH_HINT =
  "Version mismatch. Try syncing the client and server to the same T3 Trade version.";
  ...
  it("detects remote desktop-app update support from config descriptors", () => {
    const descriptor = (desktopAppUpdate?: boolean) => ({
      environment: {
        ...
        // The fork-required identity fields every descriptor carries
        // (see ExecutionEnvironmentDescriptor in @t3tools/contracts).
        fork: "T3 Trade" as const,
        t3UpstreamCommit: "a8e05cbb92633a1351529f2bc402071f615e5051",
        serverVersion: "9.9.9",
        capabilities: {
          repositoryIdentity: true,
