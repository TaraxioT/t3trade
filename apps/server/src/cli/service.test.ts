...
it("reports the installed service version and host paths", () => {
  assert.equal(
    ...
    [
      "T3 Trade service",
      "  Status: installed · t3trade@0.0.29",
...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `npx t3trade@0.0.29 service update`.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("npx t3trade@0.0.32-nightly.1 service update");
  expect(output).not.toContain("t3trade@latest");
});
...
it("suggests the newer CLI version when the installed service needs an update", () => {
  ...
  expect(output).toContain("npx t3trade@0.0.29 service update");
  expect(output).not.toContain("npx t3trade@0.0.28 service update");
});
...
it("reports a newer installed service and gives an exact-version repair command", () => {
  ...
  assert.include(output, "t3trade@0.0.32-nightly.1 (newer than this t3trade@0.0.31 CLI)");
  assert.include(output, "npx t3trade@0.0.32-nightly.1 service update");
  assert.notInclude(output, "npx t3trade@latest service update");
});
