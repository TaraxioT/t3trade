...
// `fork` and `t3UpstreamCommit` are required by this fork's
// ExecutionEnvironmentDescriptor (PROMPT-00). A descriptor without them fails
// to decode, and pair discovery then reports "no running server" rather than
// the decode error — so an upstream fixture must carry them.
...
describe("t3 pair", () => {
  ...
  it.effect("directs to t3 serve or t3 connect when no server is running", () =>
    Effect.gen(function* () {
      ...
      assert.include(rendered, "npx t3trade serve");
      assert.include(rendered, "npx t3trade connect");
    }).pipe(Effect.provide(NodeServices.layer)),
  );
