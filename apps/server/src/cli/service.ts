...
export function formatServiceStatus(
  ...
  if (
    ...
    return [
      "T3 Trade service",
      `  Status: installed · ${CLI_PACKAGE_NAME}@${installedVersion} (newer than this ${CLI_PACKAGE_NAME}@${cliVersion} CLI)`,
  ...
  return [
    "T3 Trade service",
    `  Status: ${status.current ? `installed · ${CLI_PACKAGE_NAME}@${installedVersion}` : "needs an update or repair"}`,
    `  Unit: ${status.unitPath}`,
    `  Logs: ${status.logPath}`,
    ...problems,
    ...(status.current
...
const serviceInstallCommand = Command.make("install", serviceReconcileFlags).pipe(
  Command.withDescription("Install T3 Trade as a background service for this user."),
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription("Stop and remove the T3 Trade background service."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          removed ? "Removed the T3 Trade service." : "T3 Trade service is not installed.",
        );
      }),
    ),
  ),
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription("Show whether the T3 Trade background service is installed."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (installed && current) {
    yield* Console.log("T3 Trade is already set up to run in the background on this machine.");
    return true;
  }
  ...
  const wanted = yield* Prompt.run(
    Prompt.confirm({
      message: installed
        ? "The installed T3 Trade service needs an update or repair. Update it now?"
        : platform === "darwin"
          ? "Run T3 Trade in the background whenever you log in to this Mac? " +
            "It stays reachable through T3 Connect while you are logged in."
          : "Run T3 Trade in the background whenever this machine boots? " +
