...
import * as TradingMissionProjection from "../trading/TradingMissionProjection.ts";
...
export const make = Effect.gen(function* () {
  ...
  const missions = yield* TradingMissionProjection.TradingMissionProjection;

  const sweep = Effect.fn("ThreadSettlementReactor.sweep")(function* () {
    const snapshot = yield* snapshots.getShellSnapshot();
    const now = DateTime.formatIso(yield* DateTime.now);
    const projects = new Map(snapshot.projects.map((project) => [project.id, project]));
    // Threads bound to a non-terminal trading mission are exempt from every
    // server-initiated settlement path (idle sweep and pull-request merge
    // alike): ending a mission closes live exposure, which is the user's
    // manual-settle decision. One projection read per sweep, not per thread.
    const missionThreadIds = new Set(
      (yield* missions.list())
        .filter((mission) => mission.status !== "revoked" && mission.status !== "completed")
        .map((mission) => mission.threadId),
    );
              ...
              Effect.gen(function* () {
                ...
                if (
                  !shouldAutoSettleThread({
                    ...
                    missionActive: missionThreadIds.has(thread.id),
                  })
                ) {
                  return;
                }
