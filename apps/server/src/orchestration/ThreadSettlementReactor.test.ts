...
interface HarnessOptions {
  ...
  /** Mission rows the sweep's exemption reads. Only threadId and status matter. */
...
function makeMission(
  threadId: string,
  status: OrchestrationTradingMission["status"],
): OrchestrationTradingMission {
  return { threadId, status } as unknown as OrchestrationTradingMission;
}
...
const makeHarness = Effect.fn("makeThreadSettlementHarness")(function* (options: HarnessOptions) {
  ...
  const dependencies = Layer.mergeAll(
    ...
    Layer.mock(TradingMissionProjection)({ list: () => Effect.succeed(options.missions ?? []) }),
...
describe("ThreadSettlementReactor", () => {
  ...
  // Fork: server-initiated settlement never ends a live trading mission —
  // the idle sweep must skip a thread whose mission is non-terminal, while a
  // thread whose mission completed settles like any other idle thread.
  it.effect("skips threads bound to live trading missions", () =>
    Effect.scoped(
      Effect.gen(function* () {
        yield* TestClock.setTime(Date.parse(NOW));
        const fixture = yield* makeHarness({
          snapshot: makeSnapshot([
            makeThread("idle", { branch: "idle-feature" }),
            makeThread("mission-live", { branch: "mission-live-feature" }),
            makeThread("mission-done", { branch: "mission-done-feature" }),
          ]),
          branchPullRequest: () => Effect.succeed(null),
          missions: [
            makeMission("mission-live", "position_open"),
            makeMission("mission-done", "completed"),
          ],
        });
        ...
        yield* Effect.gen(function* () {
          ...
          assert.deepStrictEqual(
            ...
            [ThreadId.make("idle"), ThreadId.make("mission-done")],
          );
        }).pipe(Effect.provide(fixture.layer));
      }),
    ),
