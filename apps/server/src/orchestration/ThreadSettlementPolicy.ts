...
export function shouldAutoSettleThread(input: {
  ...
  /** A thread bound to a live trading mission is never settled by the
   * server: revoking a mission (and closing its position) is the user's
   * manual-settle decision, not a sweep's. Supplied by the reactor from the
   * mission projection; the policy stays pure. */
  readonly missionActive: boolean;
}): boolean {
  const { thread, pullRequest } = input;
  if (input.missionActive) return false;
