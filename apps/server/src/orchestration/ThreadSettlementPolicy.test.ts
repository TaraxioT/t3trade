...
const decide = (
  ...
  settings: { days?: number | null; merge?: boolean; missionActive?: boolean } = {},
) =>
  shouldAutoSettleThread({
    ...
    missionActive: settings.missionActive ?? false,
  });
...
describe("shouldAutoSettleThread", () => {
  ...
  // Fork: a live trading mission holds exposure the sweep must not close.
  // Ending a mission is the user's manual-settle decision, never the
  // server's — not by inactivity, and not by a merged pull request.
  it("never settles a thread bound to an active trading mission", () => {
    expect(decide(makeThread(), null, { missionActive: true })).toBe(false);
    expect(decide(makeThread(), null, { missionActive: true, days: null })).toBe(false);
    expect(decide(makeThread(), { state: "merged", updatedAt: NOW }, { missionActive: true })).toBe(
      false,
    );
    expect(decide(makeThread(), { state: "closed", updatedAt: NOW }, { missionActive: true })).toBe(
      false,
    );
  });
});
