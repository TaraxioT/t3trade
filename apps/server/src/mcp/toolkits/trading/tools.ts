...
export const TradingHypothesisTool = Tool.make("trading_hypothesis", {
  description:
    "The durable record of an idea. `save` {title, thesis} files it as v1; `revise` {hypothesisId, thesis, note} writes the next version. Pass `hypothesisId` ALONE to trading_backtest or trading_validate arm and it runs that idea's current version. `show` gives versions, runs and validations; `observe` {hypothesisId} puts a non-trading watcher on this chat that narrates its validations. Also `list` `shelve` `conclude` `alert_when_setup`. Never places an order. Menu: trading_hypothesis({})",
...
export const TradingEventsTool = Tool.make("trading_events", {
  description:
    'The external calendar a thesis anchors on. record {name, occurrences:[{start, end?, label?, source}]} stores dates you researched here, one source URL each, never joined or invented; date-only fields are date precision, and a timed start needs its timed end. add, list, show, retire. import_external {name, sourceKind?} captures the source once (default github-releases; generated adapter via "generated:<sourceId>", declared with trading_forge declare_source) and records retained releases — publication instants, tag labels, release URL as source, first-observed windows; publication is not availability. study {eventSetId, market, interval?, horizonBars?, entryBasis?, metric?, direction?} reads the archive, or Graph pool prices via graphSource {poolId}: interval 1m|3m|5m|15m|1h|4h|1d, horizonBars 1..500 (default 30); coarsest interval — finer intervals start later; a four-week daily study is {interval: "1d", horizonBars: 28}; per-occurrence forward returns vs an every-bar baseline, uncovered rows reported; path_extrema {direction} is the hindsight-perfect post-entry extremum, never realizable; no fees, no sizing. study_graph {eventSetId, market, graphStudy:{poolId, variants:[{label, anchor, leadMs|tailMs, horizonBars, entryBasis?}], intervalMs?, poolDataStartMs?}} compares pre/post-event entry/holding variants over Graph pool prices — per-occurrence returns, entry-day and lead net flow, coverage, applies-now vs the next occurrence; every named variant runs and is retained. Menu: trading_events({})',
...
export const TradingForgeTool = Tool.make("trading_forge", {
  description:
    'Build and install a Forge capability over the approved Uniswap pools — you write the four artifacts, the HOST compiles, tests, hashes and installs. inspect_sources shows the pools and a sample; prepare returns the authoring brief; check runs containment over staged files (a failed build never installs); install is a CAS step needing expectedActiveVersion. arm/disarm gate a detector standing — installation is NOT arming, an un-armed detector never evaluates, the scheduler turns on its own; evaluate queues one v2 run; status reports jobs, data and detector separately, plus registered substreams sources. cancel/pause/resume/uninstall are direct user controls. propose_pool records a pool for HUMAN approval — approve_pool is always refused, an agent cannot self-approve. Execution: quote prices a swap over an approved route; propose_envelope records the grant for HUMAN approval — approve_envelope always refused; evaluate_policy turns approved evidence into one proposal; envelope shows grant, proposals, intents, budget; swap prepares and attempts submission — no signer is authorized, so it refuses and nothing executes. protected_swap (proposalId, quote, deadlineUnix) atomically ADMITS a mainnet protected swap — reserves input and worst-case fees, persists the immutable intent; signing and broadcast live only in the separately approved funded lane. declare_source (sourceSpec: url, transformRef to an INSTALLED capability with a network-less transform.ts, outputEventSchema, capturePolicy) registers a generated adapter — the host alone fetches under its URL allowlist; capture_source (sourceId) runs ONE bounded fetch, sandbox parse and schema validation; revisions keep publication and first observation distinct. Menu: trading_forge({action:"status"}).',
