...
export const TradingEventsTool = Tool.make("trading_events", {
  description:
    'The external calendar a thesis anchors on. record {name, occurrences:[{start, end?, label?, source}]} stores dates you researched in this chat, each with one source URL, never several joined or invented; fetches nothing. An instantaneous event uses the same instant for start and end; a date-only source keeps date precision — never invent a midnight or a 24h span. add, list, show, retire. import_external {name, sourceKind?} captures the external source once (default github-releases; a generated adapter via sourceKind "generated:<sourceId>" — declare it with trading_forge declare_source first) and records its retained releases as a set — publication instants, tag labels, release URL as source — naming skipped documents, capture failures, and when this server first observed them: publication is not availability. study {eventSetId, market, interval?, horizonBars?, entryBasis?, metric?, direction?} reads the archive: interval 1m|3m|5m|15m|1h|4h|1d, horizonBars 1..500 (default 30); a four-week daily study is {interval: "1d", horizonBars: 28}. Pick the coarsest interval that covers the horizon: finer intervals start later in the archive. Reports per-occurrence forward returns vs an every-bar baseline, and how many the archive can see; metric path_extrema {direction short|long} adds the post-entry extremum (a short: its lowest low) and its excursion, hindsight-perfect, never a realizable result; no fees, no sizing. To show a study on the graph, call trading_chart publish_event_study with the same parameters. Menu: trading_events({})',
...
export const TradingForgeTool = Tool.make("trading_forge", {
  description:
    'Build and install a Forge capability over the approved Uniswap pools — you write the four artifacts with your own file tools, the HOST compiles, tests, hashes and installs. inspect_sources shows the pools and a real sample; prepare returns the authoring brief; check runs containment over staged files (a failed or cancelled build never installs); install is a CAS step needing expectedActiveVersion. arm/disarm gate a detector standing — installation is NOT arming, an un-armed detector never evaluates, the scheduler turns on its own; evaluate queues one v2 run on demand (v1 is install-driven); status reports jobs, data and detector view separately. cancel/pause/resume/uninstall are direct user controls. propose_pool records a pool for HUMAN approval — approve_pool is always refused here, an agent cannot self-approve. Execution: quote prices an exact-input swap over an approved route; propose_envelope records the grant for HUMAN approval — approve_envelope is always refused here; evaluate_policy turns approved evidence into one proposal; envelope shows grant, proposals, intents, budget; swap prepares the transaction and attempts submission — no signer is authorized, so it refuses and nothing executes. protected_swap (proposalId, quote, deadlineUnix) atomically ADMITS a mainnet protected swap — reserves input and worst-case fees under the approved envelope and persists the immutable intent; signing and broadcast are never reachable here, only through the separately approved funded lane. declare_source (sourceSpec JSON: url, transformRef to an INSTALLED capability whose transform.ts parses documents network-less, outputEventSchema, capturePolicy) registers a generated external-source adapter — the host alone fetches, under its URL allowlist, with credentials injected only by the host; capture_source (sourceId) runs ONE bounded fetch+sandbox parse+schema validation and records revisions with provenance (publication time vs first observation kept distinct). Menu: trading_forge({action:"status"}).',
