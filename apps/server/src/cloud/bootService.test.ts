...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    unitPath: "/home/theo/.config/systemd/user/t3trade.service",
  });
...
const macPlan = {
  ...
  unitPath: "/Users/theo/Library/LaunchAgents/com.t3tools.t3trade.service.plist",
};
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable t3trade.service") control.enabled = true;
      if (!failed && command === "systemctl --user restart t3trade.service") control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop t3trade.service" ||
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop t3trade.service");
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now t3trade.service")).toEqual(
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? ["systemctl --user stop t3trade.service", "systemctl --user restart t3trade.service"]
            : [
                "launchctl bootout --wait gui/501/com.t3tools.t3trade.service",
  ...
  it.effect("restarts an installed service when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(
        ...
        "systemctl --user stop t3trade.service",
        "systemctl --user daemon-reload",
        "systemctl --user restart t3trade.service",
      ]);
    }),
  );
    ...
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith(
          path.join("Library", "LaunchAgents", "com.t3tools.t3trade.service.plist"),
        ),
      ).toBe(true);
      ...
      expect(timeouts.get("launchctl bootout --wait gui/501/com.t3tools.t3trade.service")).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/com.t3tools.t3trade.service",
        "launchctl enable gui/501/com.t3tools.t3trade.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = "launchctl bootout --wait gui/501/com.t3tools.t3trade.service";
