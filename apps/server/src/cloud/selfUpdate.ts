  ...
  const update: ServerSelfUpdate["Service"]["update"] = Effect.fn(
    ...
    if (capability === "desktop-managed") {
      ...
      return yield* failWith(
        "This server is managed by the T3 Trade desktop app on its machine; update the desktop app to update it.",
      );
    }
    if (capability === null) {
      return yield* failWith(
        "Remote updates require the T3 Trade background service. Run `t3 service install` on the server machine.",
      );
    }
