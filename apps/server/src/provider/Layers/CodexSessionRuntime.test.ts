...
describe("T3 browser developer instructions", () => {
  ...
  it("prefers the product-native preview tools in both collaboration modes", () => {
    for (const mode of ["default", "plan"] as const) {
      ...
      NodeAssert.match(instructions, /t3-trade/);
...
describe("hasConfiguredMcpServer", () => {
  it("detects inline Codex MCP configuration arguments", () => {
    ...
    NodeAssert.equal(
      hasConfiguredMcpServer(["-c", 'mcp_servers.t3-trade.url="http://127.0.0.1/mcp"']),
      true,
    );
  });
});
  ...
  it("keeps launch args when explicit app-server args are provided", () => {
    NodeAssert.deepStrictEqual(
      codexSessionAppServerArgs(
        ["-c", "mcp_servers.t3-trade.url=http://127.0.0.1/mcp"],
      ...
      [
        ...
        "foo",
        "-c",
        "mcp_servers.t3-trade.url=http://127.0.0.1/mcp",
      ],
    );
  });
});
    ...
    Effect.gen(function* () {
      ...
      for (const runtimeMode of Object.keys(expectations) as Array<keyof typeof expectations>) {
        ...
        const client = {
          ...
          raw: {
            request: () => Effect.die("A fresh mission thread must not resume"),
          },
        };
