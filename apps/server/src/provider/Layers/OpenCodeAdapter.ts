...
interface OpenCodeSessionContext {
  ...
  /**
   * Serialises prompt admission for this session. The `activeTurnId`
   * start-vs-steer read/write pair happens inside the permit, so two
   * same-tick prompts (a mission's bootstrap wake landing on top of the
   * user's own message) cannot both mint a turn id and emit `turn.started`.
   */
