...
import { TRADING_TOOL_POLICY, WORKSPACE_TRADING_PREAMBLE } from "../TradingSessionProfile.ts";
...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("puts the command text last so attachments do not suppress expansion", () => {
    ...
    return Effect.gen(function* () {
      ...
      // The first turn of the session instance carries the workspace trading
      // prefix ahead of the command text; the turns after it carry none.
      const firstCommandBlock = {
        type: "text" as const,
        text: `${WORKSPACE_TRADING_PREAMBLE}\n\n${TRADING_TOOL_POLICY}\n\n/flow-patterns hello`,
      };

      assert.deepEqual(prompts[0]?.message.content, [firstCommandBlock]);
