...
it.layer(OpenCodeAdapterTestLayer)("OpenCodeAdapterLive", (it) => {
  ...
  it.effect("treats a post-stop session.error as a clean stop, not a runtime error", () => {
    ...
    // The echo is a subscribed event gated on a promise the abort releases,
    // so the event pump cannot drain it before the stop sets `stopped`.
    let deliverStopEcho: (() => void) | undefined;
    const stopEcho = new Promise<unknown>((resolve) => {
      deliverStopEcho = () =>
        resolve({
          type: "session.error",
          properties: {
    ...
    runtimeMock.state.subscribedEvents = [stopEcho];
    ...
    return Effect.gen(function* () {
      ...
      NodeAssert.equal(
        eventTypes.some((type, index) => {
          const payload = recorded[index]?.payload;
          return (
            type === "turn.completed" &&
            payload !== undefined &&
            "state" in payload &&
            (payload as { state: string }).state === "failed"
          );
        }),
        false,
