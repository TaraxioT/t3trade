...
export const DESKTOP_FILE_EXCLUSIONS = [
  // T3 Trade always passes the user's installed Claude executable to the SDK,
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "T3 Trade (Nightly)"
    : (desktopPackageJson.productName ?? "T3 Trade");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "T3-Trades-${version}-${arch}.${ext}",
  ...
  if (platform === "mac") {
    ...
    buildConfig.mac = {
      ...
      // An unsigned build is ad-hoc signed by the `afterPack` hook above.
      //
      ...
      //
      // Upstream's batched `sign` hook only runs on the signed path, which the
      // fork does not take today; it is kept so the seam stays small.
      ...(signed ? { sign: path.join(repoRoot, "scripts/sign-macos.ts") } : { identity: null }),
      ...(macPasskeySigning
        ? {
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "T3 Trade desktop build",
    author: "T3 Tools",
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  Command.withDescription("Build a desktop artifact for T3 Trade."),
